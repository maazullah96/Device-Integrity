...
const DeviceTable = ({ state }) => {
  ...
  const storeDevices = async () => {
    ...
    const deviceIds = devices.map(device => ethers.utils.formatBytes32String(device.deviceId));
    ...
    if (state.contract && state.contract.storeDevices) {
      let transaction = await state.contract.storeDevices(deviceIds, staticParams, dynamicParams);
      await transaction.wait();
      console.log("Transaction is done");
    } else {
          ...
          {devices.map((device,index) => (
            <tr key={device.deviceId}>
              ...
              <td>
                <Table bordered>
                  <tbody>
                    {Object.entries(device.staticParams).map(([key, value]) => (
                      <tr key={key}>
                       <td >
              ...
              <td>
                <Table bordered>
                  <tbody>
                    {Object.entries(device.dynamicParams).map(([key, value]) => (
                      <tr key={key}>
